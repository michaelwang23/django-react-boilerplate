...
- [ ] Start your project using:
    ```
    django-admin startproject dish --extension py,yml,json --name Procfile,Dockerfile,README.md,.env.example,.gitignore,Makefile --template=https://github.com/vintasoftware/django-react-boilerplate/archive/boilerplate-release.zip
    ```
...
- [ ] Add an email address to the `ADMINS` settings variable in `dish/backend/dish/settings/base.py`
- [ ] Change the `SERVER_EMAIL` to the email address used to send e-mails in `dish/backend/dish/settings/production.py`
...
- Inside the `backend` folder, do the following:
  - Create a copy of `dish/settings/local.py.example`:  
  `cp dish/settings/local.py.example dish/settings/local.py`
  - Create a copy of `.env.example`:
  `cp .env.example .env`

### If you are using Docker:
- Open the `/backend/.env` file on a text editor and uncomment the line `DATABASE_URL=postgres://dish:password@db:5432/dish`
...
- Open the `/backend/.env` file on a text editor and do one of the following:
  ...
  - If you wish to use PostgreSQL locally, uncomment and edit the line `DATABASE_URL=postgres://dish:password@db:5432/dish` in order to make it correctly point to your database URL
...
- Open a new command line window and go to the project's directory
- Create a new virtualenv with either [virtualenvwrapper](https://virtualenvwrapper.readthedocs.io/en/latest/) or only virtualenv: `mkvirtualenv dish` or `python -m venv dish-venv`
...
- Make sure the virtualenv is activated `workon dish` or `source dish-venv/bin/activate`
...
- `workon dish` or `source dish-venv/bin/activate` depending on if you are using virtualenvwrapper or just virtualenv.
